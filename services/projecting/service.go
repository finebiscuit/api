...
type Service interface {
	ProjectBalanceValue(ctx context.Context, balanceID balance.ID, cur currency.Currency, since, until util.Period) (map[util.Period]decimal.Decimal, error)
}
...
func (s service) ProjectBalanceValue(ctx context.Context, balanceID balance.ID, cur currency.Currency, since, until util.Period) (map[util.Period]decimal.Decimal, error) {
	if since == until {
		return nil, fmt.Errorf("since and until cannot be equal")
	}

	if since.Time().After(until.Time()) {
		return nil, fmt.Errorf("since cannot be after until")
	}
	...
	if !ok {
		return nil, fmt.Errorf("no current value for requested currency: %s", cur)
	}
